...
const addNeedsButton = document.querySelector("#add-needs-button");
...
addNeedsButton?.addEventListener("click", () => {
  window.alert("Add Needs form can be connected next.");
});

detailButtons.forEach((button, index) => {
  button.addEventListener("click", () => {
    window.alert(`Showing detail for outcome row ${index + 1}.`);
  });
});
